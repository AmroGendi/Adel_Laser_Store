...
function renderCart(){const box=document.querySelector('#cartItems'),t=document.querySelector('#cartTotal');if(!box)return;const c=getCart();box.innerHTML=c.length?c.map(i=>{const p=products.find(x=>x.id===i.id);return `<div class="cart-item"><img src="${p.image}" alt=""><div class="cart-item-info"><a href="product.html?id=${p.id}"><b>${p.name}</b></a><small>${money(p.price)}</small><div class="qty"><button onclick="changeQty(${p.id},-1)">−</button><b>${i.qty}</b><button onclick="changeQty(${p.id},1)">+</button></div></div><button class="remove" onclick="removeProduct(${p.id})">×</button></div>`}).join(''):`<div class="cart-empty">🛒<br><br>السلة فاضية حاليًا.</div>`;if(t)t.textContent=money(total())}
